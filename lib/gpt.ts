...
> {
  ...
  for (let i = 0; i < num_tries; i++) {
    ...
    if (list_output) {
      output_format_prompt += ` If output field is a list, classify output into the best element of the list.`;
    }

    if (dynamic_elements) {
      output_format_prompt += ` Any text enclosed by < and > indicates you must generate content to replace it. Example input: Go to <location>, Example output: Go to the garden. Any output key containing < and > indicates you must generate the key name to replace it. Example input: {'<location>': 'description of location'}, Example output: {school: 'a place for education'}.`;
    }

    if (list_input) {
      output_format_prompt += ` Generate a list of JSON, one JSON for each input element.`;
    }
