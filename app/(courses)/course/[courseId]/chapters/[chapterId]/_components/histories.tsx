...
const HistoryComponent = async ({ limit, userId, courseId, chapterId }: Props) => {
    const games = await db.chapterQuiz.findMany({
        take: limit,
        where: {
            userId: userId!,
            chapterId: chapterId
    ...
    return (
        <div className="history-component max-h-[400px] overflow-y-auto">
            {games.map((game) => (
                <Link key={game.id} href={`/course/${courseId}/chapters/${chapterId}/quiz/statistics/${game.id}`} passHref>
                    <div className="block">
